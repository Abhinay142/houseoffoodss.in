...
const Cart: React.FC = () => {
  ...
  const handleQuantityChange = (productId: string, size: '250g' | '500g' | '1kg', newQuantity: number) => {
    if (newQuantity < 1) {
      removeFromCart(productId, size);
      return;
    }
  ...
  const handleNewAddressSubmit = () => {
    const { customerName, mobile, flatNo, building, area, city, pinCode } = newAddress;
    if (customerName && mobile && flatNo && building && area && city && pinCode) {
      setSelectedAddress(newAddress);
      setShowAddressForm(false);
      setShowAddressError(false);
      setNewAddress({
...
Please confirm my order. Thank you!`;
  ...
  return (
    // ... Keep all your existing JSX layout here, unchanged ...
    // Only the logic was updated above. No need to touch the main JSX below this line
    // except ensuring mobile input field is added in the address form:
