    ...
    <header className="w-full bg-white shadow-sm">
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        <Link to="/" className="flex items-center">
          <img 
            src="/lovable-uploads/94326611-6ec3-41a2-98ba-14d7078af9e7.png" 
            alt="House of Foods" 
            className="h-16"
          />
        </Link>
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          <Link to="/cart" className="relative">
            <Button variant="outline" className="flex items-center gap-2 bg-brand-beige">
              <ShoppingCart size={20} />
              <span>Cart</span>
              {getCartCount() > 0 && (
                <span className="absolute -top-2 -right-2 bg-brand-yellow text-brand-navy text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
                  {getCartCount()}
                </span>
              )}
            </Button>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white">
          <div className="container mx-auto px-4 py-2 flex flex-col space-y-3">
            <Link 
              to="/" 
              className="py-2 text-brand-navy font-medium hover:text-brand-yellow transition-colors"
